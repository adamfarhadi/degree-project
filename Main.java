...
public class Main {
    ...
    private static void testLFSL(int N, int range, int numOps, int[] numThreads, double[][] ratios, int numRuns,
            int[] list1, int[] list2) {
        System.out.println("\nTesting LFSL with N = " + N + ", range = " + range + ", numOps = " + numOps
                + ", numRuns = " + numRuns);
        System.out.println("---");
        ...
        for (int ratioSet = 0; ratioSet < ratios.length; ratioSet++) {
            ...
            for (int numThread : numThreads) {
                ...
                for (int i = 0; i < numRuns; i++) {
                    LFSL_Test test = new LFSL_Test(numThread, list1, N, range);
                ...
                for (int i = 0; i < numRuns; i++) {
                    LFSL_Test test = new LFSL_Test(numThread, list2, N, range);
                    result[i] = test.runTest(numOps, ratios[ratioSet][0], ratios[ratioSet][1], ratios[ratioSet][2]);
                }
        ...
        System.out.println("---");
        System.out.println("Done Testing LFSL");
    }
    ...
    private static void test() {
        ...
        final double[][] ratios = new double[][] { { 0.4, 0.4, 0.2 } };
        ...
        testLFSL(N, range, numOps, numThreads, ratios, numRuns, list1, list2);
    }
